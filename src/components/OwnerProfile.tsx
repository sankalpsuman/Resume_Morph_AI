    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[250] overflow-y-auto">
          ...
          >
            <div className="bg-[var(--bg-primary)] w-full max-w-6xl rounded-[48px] border border-[var(--border-color)] shadow-2xl overflow-hidden pointer-events-auto flex flex-col lg:flex-row max-h-[90vh]">
              
              {/* Left Column: Fixed Bio */}
              <div className="lg:w-80 bg-indigo-600 p-8 text-white flex flex-col justify-between shrink-0">
                <div className="space-y-8">
                  <div className="w-24 h-24 bg-white/10 backdrop-blur-lg rounded-[32px] p-0.5 relative group">
                    <div className="w-full h-full bg-white rounded-[30px] flex items-center justify-center">
                      <span className="text-3xl font-black text-indigo-600">SS</span>
                    </div>
                    <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-amber-400 rounded-full flex items-center justify-center border-4 border-indigo-600">
                      <Sparkles className="w-4 h-4 text-white" />
                    </div>
                  </div>
                  
                  <div>
                    <h2 className="text-4xl font-black tracking-tighter leading-none mb-2">Sankalp <br/> Suman</h2>
                    <p className="text-indigo-100 font-bold uppercase tracking-widest text-[10px]">AI-Engineer & QA Architect</p>
                  </div>
                  ...
                  <div className="space-y-4 pt-8 border-t border-white/10 text-sm">
                    <a href="mailto:sankalpsmn@gmail.com" className="flex items-center gap-3 hover:text-white transition-colors text-indigo-100 italic">
                      <Mail className="w-4 h-4" /> sankalpsmn@gmail.com
                    </a>
                    <div className="flex items-center gap-3 text-indigo-100 italic">
                      <Phone className="w-4 h-4" /> +91 9540446448
                    </div>
                    <a href="https://linkedin.com/in/sankalpsuman" target="_blank" className="flex items-center gap-3 text-indigo-100 italic hover:text-white">
                      <Linkedin className="w-4 h-4" /> in/sankalpsuman
                    </a>
                    <div className="flex items-center gap-3 text-indigo-100 italic">
                ...
                <button onClick={onClose} className="mt-12 flex items-center gap-2 text-xs font-black uppercase tracking-widest text-indigo-200 hover:text-white transition-colors">
                  <X className="w-4 h-4" /> Close Profile
                </button>
              </div>

              {/* Right Column: Scrollable Portfolio */}
              <div className="flex-1 overflow-y-auto p-8 md:p-14 custom-scrollbar bg-[var(--bg-secondary)]/30">
                <div className="space-y-16">
                  
                  {/* Summary */}
                  <section className="space-y-6">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/20 rounded-full text-indigo-600 text-[10px] font-black uppercase tracking-widest border border-indigo-100">
                  ...
                  <section className="space-y-8">
                    <div className="flex items-center justify-between">
                      <h3 className="text-2xl font-black tracking-tight text-[var(--text-primary)]">Featured Innovation</h3>
                      <div className="h-px flex-1 mx-6 bg-[var(--border-color)]" />
                    </div>
                    
                    <div className="bg-white dark:bg-slate-900 rounded-[40px] border border-[var(--border-color)] overflow-hidden shadow-xl group">
                       <div className="p-8 md:p-12 space-y-8">
                          <div className="flex flex-col md:flex-row gap-8 items-start">
                             <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center p-0.5 shrink-0 rotate-3 group-hover:rotate-0 transition-transform">
                             ...
                             <div className="space-y-4">
                                <div className="flex items-center gap-4">
                                   <h4 className="text-3xl font-black tracking-tighter text-[var(--text-primary)]">Resume Morph AI</h4>
                                   <span className="px-3 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest">v2.0 Production</span>
                                </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                             {[
                               ...
                               <div key={t.label} className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)]">
                                  <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-tertiary)] mb-1">{t.label}</p>
                                  <p className="text-xs font-bold text-indigo-600">{t.val}</p>
                               </div>
                             ))}
                          </div>
