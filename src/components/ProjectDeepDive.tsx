    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[200] overflow-y-auto">
          ...
          >
            <div className="bg-[var(--bg-primary)] w-full max-w-5xl rounded-[40px] md:rounded-[64px] shadow-2xl shadow-indigo-500/20 border border-[var(--border-color)] overflow-hidden pointer-events-auto flex flex-col max-h-[90vh]">
              
              {/* Header */}
              <div className="p-8 md:p-12 border-b border-[var(--border-color)] flex items-center justify-between shrink-0">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-indigo-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-indigo-200 rotate-6 shrink-0">
                    <Rocket className="w-8 h-8 text-white" />
                  </div>
                  <div>
                    <h2 className="text-3xl md:text-4xl font-black tracking-tighter text-[var(--text-primary)]">Project Intelligence</h2>
                    <p className="text-[var(--text-tertiary)] font-bold uppercase tracking-widest text-[10px] md:text-xs">Deep Dive into Architecture & Vision</p>
                  </div>
                </div>
                <button 
                  onClick={onClose}
                  className="p-4 hover:bg-[var(--bg-secondary)] rounded-2xl transition-colors group"
                >
                  <X className="w-6 h-6 text-[var(--text-tertiary)] group-hover:text-red-500 transition-colors" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 md:p-16 custom-scrollbar">
                <div className="space-y-24">
                  ...
                  <section className="space-y-12">
                    <div className="grid lg:grid-cols-2 gap-16 items-center">
                      <div className="space-y-8">
                        ...
                        <h3 className="text-5xl font-black tracking-tighter leading-none text-[var(--text-primary)]">
                          Solving the <br />
                          <span className="text-indigo-600">Resume Struggle.</span>
                        </h3>
                      ...
                      <div className="relative">
                        ...
                        <div className="relative p-8 bg-[var(--bg-secondary)] rounded-[48px] border border-[var(--border-color)] space-y-6">
                           <div className="flex gap-4">
                             <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-lg"><Globe className="w-6 h-6 text-indigo-600" /></div>
                             <div className="flex-1 space-y-2">
                               <div className="h-4 bg-[var(--bg-primary)] rounded-full w-3/4" />
                               <div className="h-4 bg-[var(--bg-primary)] rounded-full w-1/2" />
                             </div>
                           </div>
                           <div className="py-4 border-y border-[var(--border-color)] flex justify-center">
                             <Zap className="w-8 h-8 text-amber-500 animate-pulse" />
                           </div>
                           <div className="flex items-center justify-between">
                             <div className="w-12 h-12 bg-indigo-600 rounded-2xl shadow-xl shadow-indigo-100" />
                             <div className="h-6 bg-indigo-600/20 rounded-full w-32" />
                           </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <section className="space-y-12">
                    <div className="p-1 gap-1 flex flex-col md:flex-row items-stretch">
                       <div className="flex-[0.4] bg-indigo-600 p-12 rounded-[48px] text-white flex flex-col justify-between overflow-hidden relative group">
                          ...
                          <div className="space-y-6 relative z-10">
                             <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center shadow-2xl">
                                <span className="text-3xl font-black text-indigo-600">SS</span>
                             </div>
                             <div>
                               <h4 className="text-4xl font-black tracking-tighter leading-none mb-2">Sankalp <br/>Suman</h4>
                               <p className="text-indigo-100 font-bold uppercase tracking-widest text-[10px]">Lead Architect & Full‑Stack AI Engineer</p>
                             </div>
                          </div>
                          <div className="pt-8 relative z-10">
                             <div className="flex items-center gap-2 text-indigo-200">
                                <Globe className="w-4 h-4" />
                                <span className="text-sm font-bold">Bangalore, India</span>
                             </div>
                          </div>
                       </div>
                       
                       <div className="flex-[0.6] bg-[var(--bg-secondary)] p-12 rounded-[48px] border border-[var(--border-color)] space-y-8">
                          <div className="space-y-4">
                             <h4 className="text-2xl font-black text-[var(--text-primary)] tracking-tight">The Mind Behind the Morph.</h4>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             {[
                               ...
                               <div key={i} className="p-4 bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-color)]">
                                  <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-tertiary)] mb-1">{item.label}</p>
                          ...
                          <div className="flex items-center gap-4 pt-4">
                             <div className="flex -space-x-3">
                                {[1,2,3,4].map(x => (
                                  <div key={x} className="w-10 h-10 rounded-full border-4 border-[var(--bg-secondary)] bg-indigo-100 flex items-center justify-center">
                                     <Sparkles className="w-4 h-4 text-indigo-600" />
                                  </div>
                                ))}
                             </div>
                             <p className="text-xs font-bold text-[var(--text-tertiary)]">Built for high-performance AI roles</p>
                          </div>
                       </div>
                    </div>
                  </section>
                  ...
                  <section className="space-y-12">
                    <div className="text-center space-y-4">
                      <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-900/20 rounded-full text-amber-600 dark:text-amber-400 text-[10px] font-black uppercase tracking-widest border border-amber-100 dark:border-amber-800">
                        <Cpu className="w-3 h-3" />
                        Innovation: TOON
                      </div>
                      <h3 className="text-4xl md:text-6xl font-black tracking-tighter text-[var(--text-primary)] leading-none">
                        Token-Oriented Object Notation
                      </h3>
                    </div>

                    <div className="grid md:grid-cols-2 gap-8">
                       <div className="p-10 bg-slate-50 dark:bg-slate-900/40 rounded-[32px] border border-slate-200 dark:border-slate-800 space-y-6">
                          <div className="flex items-center justify-between">
                            <h4 className="text-xl font-black text-slate-400">Traditional JSON</h4>
                            <span className="px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-black uppercase tracking-widest">Inefficient</span>
                          </div>
                          <pre className="text-xs font-mono text-slate-500 leading-relaxed overflow-x-auto p-4 bg-white dark:bg-black/20 rounded-2xl border border-slate-100 dark:border-slate-800">
{`{
  "personalInfo": {
    "name": "John Doe",
    "email": "john@doe.com"
  ...
  "experience": [
    {
      ...
      "role": "Engineer"
                          ...
                          <p className="text-sm text-slate-500 font-medium italic">High overhead due to redundant keys, quotes, and braces. Increases token usage and latency in LLM flows.</p>
                       </div>

                       <div className="p-10 bg-indigo-50 dark:bg-indigo-900/20 rounded-[48px] border border-indigo-200 dark:border-indigo-800 space-y-6 relative overflow-hidden group">
                          ...
                          <div className="flex items-center justify-between relative z-10">
                            <h4 className="text-xl font-black text-indigo-600">Modern TOON</h4>
                            <span className="px-3 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest">Optimized</span>
                          </div>
                          <pre className="relative z-10 text-xs font-mono text-indigo-600 dark:text-indigo-400 leading-relaxed overflow-x-auto p-4 bg-white dark:bg-black/40 rounded-2xl border border-indigo-100 dark:border-indigo-900/50">
{`[RESUME]
  [PI]n:John Doe|e:john@doe.com[/PI]
  [EXP][ITEM]c:Google|r:Engineer[/ITEM][/EXP]
[/RESUME]`}
                          </pre>
                          <p className="text-sm text-indigo-700 dark:text-indigo-400 font-bold p-3 bg-white/50 dark:bg-black/20 rounded-xl relative z-10">
