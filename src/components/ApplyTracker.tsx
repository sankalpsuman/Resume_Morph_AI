...
const STATUS_COLORS: Record<Status, { bg: string, text: string, dot: string }> = {
  ...
  'Wishlist': { bg: 'bg-gray-50', text: 'text-gray-600', dot: 'bg-gray-500' }
};
...
export default function ApplyTracker() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-12 md:py-20 space-y-12">
      {/* Header & Stats */}
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
        <div className="space-y-4">
          <div className="w-16 h-16 bg-gray-900 rounded-[28px] flex items-center justify-center shadow-2xl shadow-gray-200">
            <Briefcase className="text-white w-8 h-8" />
          </div>
          <h1 className="text-4xl font-black text-gray-900 tracking-tight">Apply Tracker</h1>
        ...
        <div className="flex gap-4 p-2 bg-white border border-gray-100 rounded-[32px] shadow-xl shadow-gray-100/50">
          {[
            { label: 'Total', count: stats.total, color: 'text-gray-900' },
            { label: 'Active', count: stats.active, color: 'text-blue-600' },
            { label: 'Interviews', count: stats.interviews, color: 'text-amber-600' },
            { label: 'Offers', count: stats.offers, color: 'text-green-600' }
          ].map((stat, i) => (
            <div key={i} className="px-6 py-4 rounded-3xl hover:bg-gray-50 transition-all text-center">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative group">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-indigo-600 transition-colors" />
          <input 
            type="text" 
            placeholder="Search company or role..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-16 pr-8 py-5 bg-white border border-gray-100 rounded-[24px] text-sm font-bold shadow-sm focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all"
          />
        </div>
          ...
          {['All', 'Applied', 'Interview', 'Offer', 'Rejected', 'Wishlist'].map((s) => (
            <button
              ...
              className={cn(
                ...
                filter === s ? "bg-gray-900 text-white shadow-lg" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50"
              )}
            >
              {s}
            </button>
      ...
      <div className="grid gap-6">
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            <div className="py-40 text-center space-y-6 bg-white border border-dashed border-gray-200 rounded-[48px]">
              <div className="w-20 h-20 bg-gray-50 rounded-[32px] flex items-center justify-center mx-auto">
                <Target className="text-gray-200 w-10 h-10" />
              </div>
              <p className="text-sm font-black text-gray-300 uppercase tracking-widest">No matching applications found</p>
            </div>
          ) : (
            filteredApps.map((app) => (
              <motion.div
                ...
                className="group bg-white border border-gray-100 rounded-[36px] p-6 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 flex flex-col md:flex-row items-center gap-8"
              >
                <div className="w-16 h-16 bg-gray-50 rounded-3xl flex items-center justify-center shrink-0 text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                  <Briefcase className="w-8 h-8" />
                </div>
                
                <div className="flex-1 space-y-2 text-center md:text-left">
                  <div className="flex items-center justify-center md:justify-start gap-3">
                    <h3 className="text-xl font-black text-gray-900 tracking-tight">{app.role}</h3>
                ...
                <div className="flex items-center gap-2">
                  <select 
                    ...
                    className="px-4 py-3 bg-gray-50 border-none rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-500/10 cursor-pointer"
                  >
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-white rounded-[48px] shadow-2xl overflow-hidden p-10 space-y-10"
            >
              <div className="space-y-4">
                <h2 className="text-3xl font-black text-gray-900 tracking-tight">Add New Job</h2>
                <p className="text-sm font-medium text-gray-400 uppercase tracking-widest">Keep your search focused</p>
              </div>
              ...
              <form onSubmit={handleAdd} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full p-4 bg-gray-50 rounded-2xl font-bold text-sm outline-none focus:ring-4 focus:ring-indigo-500/5 focus:bg-white transition-all border border-transparent focus:border-indigo-100"
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <button 
                  ...
                  className="w-full py-5 bg-indigo-600 text-white rounded-[24px] font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all"
                >
                  Save Application
                </button>
              </form>
